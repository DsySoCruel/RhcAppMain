//
//  OrderListViewCheXiangControllerView.m
//  RHCApp
//
//  Created by daishaoyang on 2018/10/14.
//

#import "OrderListViewCheXiangControllerView.h"
#import "OrderChexiangListCell.h"
#import "OrderLisetModel.h"
#import "OrderChexiangDetailController.h"

static NSString *kOrderChexiangListCell = @"OrderChexiangListCell";

@interface OrderListViewCheXiangControllerView ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong) UITableView *tableView;
@property (nonatomic,assign) NSInteger cape;//分页筛选（上提加载）
@property (nonatomic,strong) NSMutableArray *dataArray;
@end

@implementation OrderListViewCheXiangControllerView
...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[OrderChexiangListCell class] forCellReuseIdentifier:kOrderChexiangListCell];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    OrderChexiangListCell *cell = [tableView dequeueReusableCellWithIdentifier:kOrderChexiangListCell];
    cell.model = self.dataArray[indexPath.row];
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    OrderChexiangDetailController *vc = [OrderChexiangDetailController new];
...
- (void)loadNewsData{
    ...
    WeakObj(self);
    [[NetWorkManager shareManager] POST:USER_CarriageOrderList parameters:parames successed:^(id json) {
        if (json) {
